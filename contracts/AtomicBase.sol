...
abstract contract AtomicBase {
    ...
    uint256 private constant SLIPPAGE_DENOMINATOR = 10_000;
     ...
     */
    function _checkSlippage(uint256 expected, uint256 actual, uint256 maxSlippageBps) internal pure {
        if (actual < expected) {
     ...
     */
    function _calculateMinimumOutput(uint256 expectedOutput, uint256 slippageBps) internal pure returns (uint256) {
    ...
    function _safeTransferFrom(address token, address from, address to, uint256 amount) internal {
        ...
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(selector, from, to, amount));
    ...
    function _safeApprove(address token, address spender, uint256 amount) internal {
        ...
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(selector, spender, amount));
